...
const ConsumptionCreate = () => {
  ...
  if (isFirstMonthOfQuarter) {
    ...
    for (let month = prevQuarterStartMonth; month < prevQuarterStartMonth + 3; month++) {
      displayMonths.push({
        month,
  ...
  const handleMonthChange = (event) => {
    ...
    const selectedMonthData = displayMonths.find(m => m.month === selectedMonth);
                    ...
                    <Grid item xs={6}>
                      ...
                      >
                        ...
                        <Select
                          ...
                          disabled={isLoading}
                        >
                          {displayMonths.map(({ month, year, isCurrent, showYear }) => {
                            const monthDate = new Date(year, month - 1, 1);
                            const monthName = monthDate.toLocaleString('default', { month: 'long' });
                            
                            return (
                              <MenuItem key={`${year}-${month}`} value={month}>
                                {monthName}
                                {isCurrent && ' (Current)'}
                                {showYear && ` (${year})`}
                              </MenuItem>
              ...
              <Grid item xs={12} md={8}>
                ...
                <Box sx={{
                  ...
                  <label htmlFor="file-upload">
                    ...
                    >
                      <Box className="upload-icon" sx={{ transition: 'transform 0.3s ease' }}>
                        <UploadFileIcon color="action" sx={{ fontSize: 48, mb: 2, color: 'primary.main', opacity: 0.8 }} />
                      </Box>
                      <Typography variant="subtitle1" gutterBottom sx={{ fontWeight: 500 }}>
                        Drag & drop files here
                      </Typography>
                      <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
                        or <Box component="span" sx={{ color: 'primary.main', textDecoration: 'underline', fontWeight: 500 }}>browse</Box> to choose files
                      </Typography>
