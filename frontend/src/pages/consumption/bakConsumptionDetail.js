...
const ConsumptionDetail = () => {
  ...
  // Función para determinar el color de texto con mejor contraste (blanco o negro)
  const getContrastText = (hexColor) => {
    // Si el color es transparente o inválido, devolver negro por defecto
    ...
    // Si el brillo es mayor a 0.5, usar texto oscuro, de lo contrario usar texto claro
  ...
  const getStatusColor = (status) => {
    ...
    const colors = {
      in_progress: { main: '#ed6c02', contrastText: getContrastText('#ed6c02') },  // naranja
      sent: { main: '#0288d1', contrastText: getContrastText('#0288d1') },        // azul
      rejected: { main: '#d32f2f', contrastText: getContrastText('#d32f2f') },    // rojo
      completed: { main: '#2e7d32', contrastText: getContrastText('#2e7d32') },   // verde
      default: { main: '#9e9e9e', contrastText: getContrastText('#9e9e9e') }      // gris
    };
  ...
  // Filtrar opciones de estado para excluir el estado actual
  const statusOptions = useMemo(() => 
    allStatusOptions.filter(option => option.value !== safeConsumption?.status),
    [safeConsumption?.status]
  );

  // Filtrar opciones de estado para excluir el estado actual
  const filteredStatusOptions = useMemo(() => 
    statusOptions.filter(option => option.value !== safeConsumption?.status),
    [safeConsumption?.status, statusOptions]
  );
  ...
  const handleEdit = () => {
    console.log('Editing consumption:', id);
    // Here would be the edit logic
  };
            ...
            <Grid item>
              ...
              <Box display="flex" alignItems="center" mt={1}>
                ...
                <span style={{ marginLeft: '8px' }}>
                  {safeConsumption?.status || 'Cargando...'}
                </span>
              </Box>
                        ...
                        <Box>
                          ...
                          >
                            {statusOptions.map((option) => {
                              ...
                              const isSelected = option.value === safeConsumption?.status;
                              const isDisabled = option.value === safeConsumption?.status;
                              ...
                              return (
                                <MenuItem
                                  ...
                                  sx={{
                                    '&.Mui-selected': {
                                      backgroundColor: 'rgba(0, 0, 0, 0.08)', // Color fijo para selected
                                      '&:hover': {
                                        backgroundColor: 'rgba(0, 0, 0, 0.04)', // Color fijo para hover
                                      }
                                    },
                                    '&.Mui-disabled': {
                                      opacity: 1,
                                    ...
                                    minWidth: '180px',
                                    py: 1.5,
                                    color: isSelected ? statusColor.main : 'inherit',
                                    '&:hover': {
