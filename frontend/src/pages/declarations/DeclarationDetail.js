...
const DeclarationDetail = () => {
  ...
  const [confirmDialog, setConfirmDialog] = useState({
    ...
    content:
      "Are you sure you want to mark this declaration as completed? This action cannot be undone.",
    onConfirm: null,
  });
  ...
  const fetchReportData = useCallback(async () => {
    ...
    try {
      ...
      const response = await getQuarterlyReportDetails(
        companyId,
        quarter,
        year,
      );
  ...
  const stateSummary = useMemo(() => {
    if (!reportData.state_totals || reportData.state_totals.length === 0)
      return [];

    // Calculate totals
    const totalMiles = reportData.state_totals.reduce(
      (sum, state) => sum + (parseFloat(state.total_miles) || 0),
      0,
    );

    const totalGallons = reportData.state_totals.reduce(
      (sum, state) => sum + (parseFloat(state.total_gallons) || 0),
      0,
    );
    ...
    const summary = reportData.state_totals.map((state) => ({
      ...
      mpg: calculateMPG(
        parseFloat(state.total_miles) || 0,
        parseFloat(state.total_gallons) || 0,
      ).toFixed(2),
      percentage:
        totalMiles > 0
          ? ((parseFloat(state.total_miles) || 0) / totalMiles) * 100
          : 0,
    }));
  ...
  const vehicleStateTableData = useMemo(() => {
    if (!reportData.individual_reports) {
      return {
        ...
        months: [],
        quarters: [],
        generalTotal: { miles: 0, gallons: 0, mpg: "0.00" },
      };
    }

    const reportsToProcess =
      filters.vehicle !== "all"
        ? reportData.individual_reports.filter(
            (r) => r.vehicle_plate === filters.vehicle,
          )
    ...
    reportsToProcess.forEach((report, index) => {
      ...
      report.state_data.forEach((state, stateIndex) => {
        ...
        stateData.mpg =
          stateData.totalGallons > 0
            ? (stateData.totalMiles / stateData.totalGallons).toFixed(2)
            : "0.00";
      ...
      vehicle.mpg =
        vehicle.totalGallons > 0
          ? (vehicle.totalMiles / vehicle.totalGallons).toFixed(2)
          : "0.00";
    });
    ...
    const generalMPG =
      generalTotalGallons > 0
        ? (generalTotalMiles / generalTotalGallons).toFixed(2)
        : "0.00";
    ...
    setConfirmDialog({
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          const quarterlyUpdateResponse = await updateQuarterlyReportStatus(
            quarterlyReportId,
            "completed",
          );
          console.log(
            "8. Respuesta de actualización del reporte trimestral:",
          ...
          const individualUpdates = await Promise.all(
            reportIds.map((reportId) => {
              ...
              return api
                .patch(`/v1/ifta-reports/${reportId}/status`, {
                  status: "completed",
                })
                .then((res) => {
                ...
                .catch((error) => {
                  console.error(
                    `   - Error actualizando reporte ${reportId}:`,
                    error,
                  );
                  throw error;
                });
          ...
          setAlert({
            open: true,
            message:
              "La declaración y sus reportes individuales han sido marcados como completados exitosamente",
            severity: "success",
          });
          ...
          setAlert({
            open: true,
            message:
              "Error al actualizar la declaración: " +
              (error.response?.data?.message || error.message),
            severity: "error",
          });
        } finally {
  ...
  const filteredReports = useMemo(() => {
    ...
    return reportData.individual_reports.filter((report) => {
      ...
      if (
        filters.vehicle !== "all" &&
        report.vehicle_plate !== filters.vehicle
      ) {
        return false;
      }
  ...
  const filteredTotals = useMemo(() => {
    if (!reportData.individual_reports)
      return { totalMiles: 0, totalGallons: 0, avgMPG: 0 };
    ...
    return reportsToProcess.reduce(
      (acc, report) => {
  ...
  const reportSummary = useMemo(() => {
    const totalMiles =
      reportData.state_totals?.reduce(
        (sum, state) => sum + (parseFloat(state?.total_miles) || 0),
        0,
      ) || 0;
    ...
    const avgMPG =
      totalGallons > 0 ? (totalMiles / totalGallons).toFixed(2) : 0;
          ...
          <TableHead>
            ...
            <TableRow>
              ...
              {quarterEntries.map(([quarter, months]) => {
                ...
                const mpg =
                  quarterTotal.gallons > 0
                    ? (quarterTotal.miles / quarterTotal.gallons).toFixed(2)
                    : "0.00";
            ...
            <TableRow>
              {vehicleStateTableData.months.map((month, idx) => (
                ...
                >
                  ...
                  >{`${month.monthName} ${month.year}`}</div>
                  <div>
                    <span>MILES</span>
                    <span>GAL</span>
                  </div>
                ...
                vehicleRows.push(
                  ...
                  >
                    ...
                    {vehicleStateTableData.months.flatMap((month) => {
                      ...
                      const mpg =
                        monthData.gallons > 0
                          ? (monthData.miles / monthData.gallons).toFixed(2)
                          : "0.00";
  ...
  const companyName =
    companyInfo?.name || currentUser?.company_name || "Company Name";
  ...
  return (
    <Box sx={{ p: 3, overflow: "hidden" }}>
      {/* Report Header */}
      <Card
        sx={{ mb: 3, backgroundColor: "#f8f9fa", border: "1px solid #e0e0e0" }}
      >
        <CardContent sx={{ py: 2, "&:last-child": { pb: 2 } }}>
          ...
          >
            <Box>
              ...
              <Typography
                variant="subtitle2"
                sx={{ color: "#7f8c8d", fontSize: "0.9rem" }}
              >
            ...
            >
              ...
              {reportData.status === "completed" && (
                <PDFDownloadLink
                  ...
                  fileName={`reporte-ifta-${
                    companyName?.replace(/\s+/g, "-").toLowerCase() || "empresa"
                  }-Q${quarter}-${year}.pdf`}
                >
              ...
              {reportData.status === "completed" ? (
                ...
                <Button
                  ...
                  startIcon={
                    updatingStatus ? <CircularProgress size={20} /> : null
                  }
                  sx={{ minWidth: "200px" }}
                >
                ...
                <TableBody>
                  {(() => {
                    ...
                    if (filters.vehicle === "all" || !filters.vehicle) {
                      ...
                      filteredStateSummary = stateSummary.filter(
                        (state) => state.state !== "TOTAL",
                      );
                      totalMiles = filteredStateSummary.reduce(
                        (sum, state) => sum + (parseFloat(state.miles) || 0),
                        0,
                      );
                      totalGallons = filteredStateSummary.reduce(
                      ...
                      const vehicleStateData = reportData.individual_reports
                        .filter(
                          (report) => report.vehicle_plate === filters.vehicle,
                        )
                      ...
                      totalMiles = statesData.reduce(
                        (sum, state) => sum + state.miles,
                        0,
                      );
                      totalGallons = statesData.reduce(
                        (sum, state) => sum + state.gallons,
                        0,
                      );
                      ...
                      filteredStateSummary = statesData.map((state) => ({
                        ...
                        percentage:
                          totalMiles > 0 ? (state.miles / totalMiles) * 100 : 0,
                      }));
                    }
                      ...
                      <>
                        ...
                        {filteredStateSummary.length > 0 && (
                          ...
                          >
                            ...
                            <TableCell align="right" sx={{ width: "20%" }}>
                              {formatNumber(totalGallons)}
                            </TableCell>
                            {isUserAdmin && (
                              <TableCell align="right" sx={{ width: "15%" }}>
                                {parseFloat(
                                  calculateMPG(totalMiles, totalGallons),
                                ).toFixed(2)}
                              </TableCell>
                            )}
                            <TableCell
                  ...
                  return Object.entries(vehicles)
                    ...
                    .map(([plate, data]) => (
                      <TableRow key={plate}>
                        ...
                        <TableCell align="right">
                          {formatNumber(data.Q2)}
                        </TableCell>
                        <TableCell align="right">
                          {formatNumber(data.Q3)}
                        </TableCell>
                        <TableCell align="right">
                          {formatNumber(data.Q4)}
                        </TableCell>
                        <TableCell align="right" sx={{ fontWeight: "bold" }}>
                ...
                {(() => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell align="right" sx={{ fontWeight: "bold" }}>
                        {formatNumber(totals.Q1)}
                      </TableCell>
                      <TableCell align="right" sx={{ fontWeight: "bold" }}>
                        {formatNumber(totals.Q2)}
                      </TableCell>
                      <TableCell align="right" sx={{ fontWeight: "bold" }}>
                        {formatNumber(totals.Q3)}
                      </TableCell>
                      <TableCell align="right" sx={{ fontWeight: "bold" }}>
                        {formatNumber(totals.Q4)}
                      </TableCell>
                      <TableCell
                        align="right"
